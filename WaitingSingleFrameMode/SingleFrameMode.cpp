...
int main(int argc, char *argv[])
{
  ...
  // The List of All Variables -- SET THESE BY SENDING A COMMAND IN CONSOLE

  double exposureTime = atof(argv[1]) * SECOND;               // Exposure time (in us)
  int gainSetting = atoi(argv[2]);                           // Gain Setting
  int offsetSetting = atoi(argv[3]);                       // Offset Setting
  double tempSetting = atoi(argv[4]);                        // Temperature of Camera
  int runTimes = atoi(argv[5]);                           // How Many Pictures To Get
